...
for j in np.arange(0,len(lstDW)):
    ...
    lstYr = [x.split('/')[-1].split('_')[7] for x in lstDW]
    lstDay = [x.split('/')[-1].split('_')[8] for x in lstDW]
...
#for i in np.arange(0,len(lstYr)):
    ...
    lstYD.append(fn)
    fn2 = glob.glob('vel/p063_r017/*16*_delcorr.tif')
    lstDC.append(fn2)
    fn3 = glob.glob('vel/p063_r017/*16*_vv.tif')
    lstVV.append(fn3)
    ...
    #a = dc.img<0.3
    #gw_ma=np.ma.masked_where(a[:,1:-1],gw.img)
    gw_ma=gw.img

    minib8,maxjb8 = b8.XY2imageij(bbminx,bbminy)
    maxib8,minjb8 = b8.XY2imageij(bbmaxx,bbmaxy)

    minigw,maxjgw = gw.XY2imageij(bbminx,bbminy)
    maxigw,minjgw = gw.XY2imageij(bbmaxx,bbmaxy)

    minidc,maxjdc = dc.XY2imageij(bbminx,bbminy)
    maxidc,minjdc = dc.XY2imageij(bbmaxx,bbmaxy)
    ...
    plt.figure()
    plt.imshow(b8.img[minjb8:maxjb8,minib8:maxib8],cmap='gray',extent=[bbminx,bbmaxx,bbminy,bbmaxy])
    plt.imshow(gw_ma[minjgw:maxjgw,minigw:maxigw],cmap='jet',alpha=0.3,extent=[bbminx,bbmaxx,bbminy,bbmaxy],vmin=-0.002,vmax=0.002)
    #plt.ion()
    #plt.show()

    plt.savefig('strain_overlay/upper_walsh/' +x+ '_p063.png')

    plt.figure()
    plt.imshow(b8.img[minjb8:maxjb8,minib8:maxib8],cmap='gray',extent=[bbminx,bbmaxx,bbminy,bbmaxy])
    plt.imshow(vv.img[minjvv:maxjvv,minivv:maxivv],cmap='jet',alpha=0.3,extent=[bbminx,bbmaxx,bbminy,bbmaxy],vmin =0.0,vmax=0.5)
    #plt.ion()
    #plt.show()

    plt.savefig('velocity_overlay/upper_walsh/' +x+ '_p063.png')

    plt.figure()
    plt.imshow(b8.img[minjb8:maxjb8,minib8:maxib8],extent=[bbminx,bbmaxx,bbminy,bbmaxy],cmap = 'gray')
    #plt.ion()
    #plt.show()

    plt.savefig('band8_overlay/upper_walsh/' +x+ '_p063.png')
