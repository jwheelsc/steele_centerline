...
lstVV = glob.glob('E:\surge_project\p063_r017_nc2tif\*_vv_*')

#for i in np.arange(0,len(lstVX)):
for i in np.arange(1):

    inputName = lstVV[i]
    fNameVV = inputName[34:87]
    print(inputName)
    print(fNameVV)    
...
#    vy=GeoImg(fNameVY, in_dir = 'vel\p063_r017')
    ...
    vv=GeoImg(fNameVV, in_dir = 'E:\surge_project\p063_r017_nc2tif')
    
    out_pixels = vv.num_pix_x
    out_lines = vv.num_pix_y
    out_bands = 1
    ...
    dst_filename = ('E/surge_project/' + fNameVV )
...
#    dst_ds.SetGeoTransform(vv.gt) # note pix_y_m typically negative
#    dst_ds.SetProjection(vv.proj)
